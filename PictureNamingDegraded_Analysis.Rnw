...
<<>>=
pic = read.csv("CompiledPictureDegraded_CSV.csv", header = TRUE, sep = ",")
...
\section {Separating Intact & Degaraded}
...
\subsection*{Histogram of trimmed}
<<>>=
ggplot(pic_degraded_trimmed, aes(x = zRT))+
...
<<>>=
main_intact = new_intact_z[,c(1,3,4,5,6,7,8,17)]
  
main_degraded = new_degraded_z[,c(1,3, 4,5, 6,7,8,17)]   
...
item_acc = group_by(new_pic_z_final, Object, PictureType )%>%
    summarise_at(vars(Accuracy, zRT_trim), mean)
...
fixed.frame.2 %>%
  ...
  ggplot(aes(x = AoA, y = pred, color = PictureType)) +
    geom_line(size = 1) + 
    xlab("Age of Acquisition") + ylab ("z-RT")+ 
  ggtitle("Model Fit: zRT by AoA And Picture Type \n After removing 6 low-accuracy items ")+
theme_classic() +
    theme(axis.text = element_text(face = "bold", size = rel(1.2)),
          axis.title = element_text(face = "bold", size = rel(1.2)),
          legend.title = element_text(face = "bold", size = rel(1.2)),
...
\section {Item Distributions}

<<>>=
ggplot(pic_withAoA_z, aes(x = zRT_trim)) +
    geom_histogram()+
   # facet_wrap(~Object)+
       xlab("Clear Mean") + ylab ("Degraded Mean")+ 
   # xlim(-0.6,0.5)+
